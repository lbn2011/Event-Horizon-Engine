...
SpirvResult compile_glsl_to_spirv(const std::string& source, bool fragment_stage,
    ...
    const EShLanguage stage = fragment_stage ? EShLangFragment : EShLangVertex;
    ...
    if (!shader.parse(GetDefaultResources(), 450, false, EShMsgDefault)) {
        result.log = join_log(shader, fragment_stage ? "frag" : "vert");
        return result;
    }
    ...
    glslang::GlslangToSpv(*program.getIntermediate(stage), result.words, &options);
...
SpirvResult compile_shader_file(const std::string& path, const std::vector<std::string>& roots,
    ...
    return compile_glsl_to_spirv(loaded.source.text, fragment_stage, defines);
}
