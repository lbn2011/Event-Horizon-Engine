# Event Horizon Engine (EHE) — 开发文档 V5.1
...
| 开发机 GPU = AMD Radeon HD 7400M（无 Vulkan、GL ≤4.1、1GB VRAM） | **开发机零 GPU 运行验证**，仅编译 + CPU 侧验证 |
...
- **颜色**：局部温度 T(r) = T_scale · F(r)^{1/4}（T_scale 为面板"盘温"）；
  ...
  **LUT 生成规格**（core/BlackbodyLut，离线工具）：
  ...
  - 每点：Planck 谱 B_λ(T) 在 [380, 780] nm 以 1 nm 步长数值积分 → 乘 CIE 1931 配色函数
    （数据表以头文件常量入库，来源注明）→ XYZ → 线性 sRGB（标准 3×3 矩阵）→ 按峰值归一
  - 输出二进制 `blackbody_lut.f32`（256×3 float32）+ PNG 预览条
  - shader 侧按 log(T) 映射 UV 采样（与生成端同一映射函数，公式写入共享头注释）
- **湍流**：hash 型 3D value noise，4 倍频 fbm 调制 ρ 与发射【建议默认】；
  噪声采样坐标随开普勒角速度 Ω(r) = r^{−3/2}（M=1）绕 z 轴差速旋转（内快外慢），
  时间由 UBO `time` 驱动。
...
- **体渲染**：raymarch 途中穿越盘体积时按发射-吸收模型前向合成：
  ...
  ```
  ε = ρ · g³ · LUT(g·T(r))；κ 为吸收系数常数【建议默认 κ=2，入 Config】；α ≥ 0.99 提前终止。
...
512×512；相机 r=15、倾角 75°、方位角 0；FOV 40°；a=0；盘 [6, 20]、disk_density=1、t_scale=1；
N_max=300、h₀=0.05、h_min=1e-3、h_max=0.5；无噪声动画（time=0）；无后处理（纯 HDR 输出做差分）。
...
| 黑洞 | 质量 M、自旋 a（M1 灰显锁 0）、盘内半径、盘外半径、盘密度、盘温 T_scale |
...
```json
{
  ...
  "blackhole": { "mass": 1.0, "spin": 0.0, "disk_r_in": 6.0, "disk_r_out": 20.0,
                 "disk_density": 1.0, "disk_t_scale": 1.0 },
